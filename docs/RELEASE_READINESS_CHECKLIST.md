...
| Required files present | `api/lib_reconciliation.php`, `api/recon_diagnostics.php`, `api/vulnerability_dashboard.php`, `daemon/recon_observations.py`, `daemon/st_software_observation_selftest.py`, maintenance CLIs + selftests under **`/opt/surveytrace/scripts/`** (see manifest), `api/lib_vulnerability_advisory_import.php`, `scripts/st_software_inventory_summary_selftest.php`, `scripts/st_software_inventory_evidence_selftest.php`, `scripts/st_software_inventory_diagnostics_selftest.php`, `scripts/st_vulnerability_correlation_selftest.php`, `scripts/st_vulnerability_dashboard_selftest.php`, `scripts/st_remove_advisory_selftest.php`, `scripts/st_vulnerability_triage_selftest.php`, `docs/TRUSTED_DATA_MODEL.md`, **`docs/samples/*.json`** (and cred-checks design docs if shipped) under `/opt/surveytrace`. |
...
| Diagnostics | `php scripts/diagnose_advisory_feeds.php` (feed ops aggregator) · `php scripts/diagnose_vulnerability_correlation.php` · `php scripts/diagnose_vulnerability_dashboard.php` · `php scripts/diagnose_vulnerability_triage.php` emit JSON; **`counts_by_priority`** in triage reflects **`affected`** joins only. |
...
| Dashboard selftest | `php scripts/st_vulnerability_dashboard_selftest.php` passes (rollup math, suppression/override counts, aging, risk bands, bounded limits, API shapes). |
| Dashboard diagnostics | `php scripts/diagnose_vulnerability_dashboard.php` emits JSON (summary, top assets, freshness, stale warnings, triage mismatches). |
| Dashboard API | `GET /api/vulnerability_dashboard.php?action=summary` returns valid JSON with `ok:true`. Confirm bounded responses for `top_assets`, `recent_findings`, `aging`, `by_severity`, `by_package`, `by_advisory`, `suppressed`, `overrides`. |
