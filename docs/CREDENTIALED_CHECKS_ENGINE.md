...
**Advisory correlation is separate from the scan:** cred checks **only** feed **normalized inventory** + summarized observations. **`asset_vulnerabilities`** rows come from **offline** PHP correlation (`run_vulnerability_correlation.php` / job consumer) comparing inventory to **locally imported** advisories. **NVD metadata imports** enrich CVE rows but use **`package_authority = metadata_only`** — they **do not** mark assets affected by themselves. **Vendor/distro** and **internal** package rules drive affected state; see [TRUSTED_DATA_MODEL.md](TRUSTED_DATA_MODEL.md) (*Local advisory correlation*) and [wiki — Vulnerability advisory operator runbook](wiki/vulnerability-advisory-runbook.md).

**Post-correlation analyst workflow (PHP/UI, not worker-written):** when migrations ship, **`asset_vulnerability_triage`**, **`vulnerability_notes`**, and **`vulnerability_activity_log`** layer **operator prioritization, triage states, suppression with expiry, and audit** on top of **`asset_vulnerabilities`** — see [TRUSTED_DATA_MODEL.md](TRUSTED_DATA_MODEL.md) (*Vulnerability triage*). The cred worker does **not** write triage rows; correlation + triage helpers run on the app database during offline jobs and web APIs. Notes are **plain text** with a hard size cap; UI must **escape** note text (no HTML trust).

**Vulnerability Dashboard (read-only, bounded):** `api/vulnerability_dashboard.php` exposes operator risk posture from correlation data. Per-asset risk rollup uses weighted scoring (critical=40, high=10, medium=3, low=1). All queries are capped (`LIMIT` ≤250). The dashboard does **not** write data, call external APIs, or trigger correlation — it is a read-only view layer on top of existing `asset_vulnerabilities` and `asset_vulnerability_triage` tables. Health: `vulnerability_dashboard` block in `/api/health.php` warns on stale correlation, no advisories, or aging criticals. See [TRUSTED_DATA_MODEL.md](TRUSTED_DATA_MODEL.md) (*Vulnerability Dashboard*).
...
**No-network test:** `python3 daemon/cred_check_package_inventory_selftest.py` (parser/sanitizer). **`python3 daemon/software_inventory_selftest.py`** (via **`php scripts/st_software_inventory_normalization_selftest.php`**) covers normalization + persist + diff. **`python3 daemon/st_software_observation_selftest.py`** remains for legacy **`software_observed`** helpers. **`php scripts/st_software_inventory_summary_selftest.php`** covers resolver rules. **`php scripts/st_software_inventory_evidence_selftest.php`** covers API field contract + bounded recon payload rules. **`php scripts/st_software_inventory_diagnostics_selftest.php`** covers stale bands + health **`trusted_data`** leak guards. **`python3 daemon/cred_check_os_release_selftest.py`** remains for os-release. **`php scripts/st_cc_normalized_preview_selftest.php`** asserts admin run-detail preview never exposes the full **`packages`** array (only **`packages_sample`**). Ops: **`php scripts/diagnose_software_inventory.php`** prints table row counts + latest active timestamp. Advisory correlation: **`php scripts/st_vulnerability_correlation_selftest.php`**, **`php scripts/st_remove_advisory_selftest.php`**, **`php scripts/import_advisories.php`**, **`php scripts/import_nvd_metadata.php`**, **`php scripts/import_distro_advisories.php`**, **`php scripts/remove_advisory.php`** (dry-run default), **`php scripts/run_vulnerability_correlation.php`**, **`php scripts/diagnose_vulnerability_correlation.php`**; bounded samples under **`docs/samples/`**. Triage layer: **`php scripts/st_vulnerability_triage_selftest.php`**, **`php scripts/diagnose_vulnerability_triage.php`**, **`php scripts/prune_vulnerability_activity.php`** (dry-run default; **activity log only**).
