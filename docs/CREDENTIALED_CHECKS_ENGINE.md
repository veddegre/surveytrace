...
- **Operational key model:** all decrypting processes (web/PHP and credential-check worker) must share the same `SURVEYTRACE_CRED_SECRET_KEY`; strict key parsing can be enforced with `SURVEYTRACE_CRED_SECRET_KEY_STRICT=1`.
...
**Software reconciliation foundations (slice 1):** after each successful **`package_inventory`** result, the worker also writes up to **128** **`software_observed`** rows (**deduped** within the run on name/version/manager; **`partial`** when the run was partial or when unique packages exceed the cap). Prior **`software_observed`** rows for the same asset + **`ssh.linux.package_inventory`** plugin are **deleted** so storage stays bounded (**latest inventory replaces** the bounded preview). **No** change to remote commands, parsing, or **`credential_check_results`** shape. **No** CVE/SBOM **from the executor** — see [TRUSTED_DATA_MODEL.md](TRUSTED_DATA_MODEL.md).

**Software reconciliation (slice 2 — PHP lazy reconcile):** on host detail (`assets.php` single-asset GET), **`st_recon_lazy_reconcile_software_inventory_summary`** derives **one** **`asset_assertions`** row per asset with **`assertion_type = software_inventory_summary`** (compact label like **`237 packages (dpkg)`**). Inputs are **`software_observed`** + latest **`package_inventory_observed`** only. **`assertion_sources`** link the summary observation (primary) plus up to **five** **`software_observed`** samples — **never** per-package assertions. **Confidence** is **`low`** \| **`medium`** based on **freshness** (**90-day** stale threshold), partial/truncated/bounded sampling — **not** CVSS or exposure. Explanation **must** state inventory is bounded and **no CVE matching** ran. **Deferred:** CVE correlation, findings, SBOM, dependency graphs, package alerts — unchanged.

**Software inventory UX / diagnostics (slice 3):** single-asset JSON adds **`software_inventory_stale`** (server-derived, aligned with the **90-day** threshold) and **`software_inventory_source`** (**credentialed SSH package inventory** label). Host modal copy distinguishes **partial/bounded** inventory from CVE analysis and states that **no findings** are generated from package inventory yet; **View software evidence** exposes only the bounded **`recon_detail.software_observed`** preview (**≤3** samples — never the full package table). **`trusted_data`** adds **`software_inventory_summary_stale_assets`**, **`software_inventory_summary_partial_assets`**, and **`software_observed_without_summary_assets`** (quiet when zero). Admin **`recon_diagnostics`** includes read-only **`software_inventory`** (collect + resolve only — **no** lazy upsert). **Deferred:** unchanged — no CVE matching, no per-package assertions, no remediation.

**Software inventory weighting / readiness (slice 4 — no new ingest):** resolver explanations spell out **why** confidence is **`medium`** vs **`low`** (fresh window, partial/truncated/bounded inventory, stale bands **`90_180`** vs **`over_180`**) and call out **`software_inventory_observation_gap`** when `package_inventory_observed` exists without **`software_observed`** corroboration (still **`medium`** when otherwise fresh+complete — diagnostics-first). Single-asset JSON adds **`software_inventory_has_bounded_observations`**, **`software_inventory_observation_gap`**, **`software_inventory_stale_band`**. **`trusted_data`** adds bounded readiness counters (stale evidence age splits **90–180d** vs **>180d**, repeat partial package-inventory assets, summaries reconciled newer than latest **`software_observed`**, summaries lacking **`software_observed`** rows). Resolver copy reserves future fusion semantics (**scanner / API / SBOM / agent** inventory) without wiring new transports or feeds. Health payloads remain **count-only** — **`st_recon_slice4_assert_health_trusted_software_diag_bounded`** guards contract tests against raw **`packages`** / result blobs on **`trusted_data`**.

**No-network test:** `python3 daemon/cred_check_slice8_pkg_selftest.py` (parser/sanitizer). **`python3 daemon/st_software_obs_slice1_selftest.py`** covers **`software_observed`** normalization and bounding. **`php scripts/st_software_inventory_slice2_selftest.php`** covers resolver rules. **`php scripts/st_software_inventory_slice3_selftest.php`** covers API field contract + bounded recon payload rules. **`php scripts/st_software_inventory_slice4_selftest.php`** covers stale bands + health **`trusted_data`** leak guards. **`python3 daemon/cred_check_slice7_selftest.py`** remains for os-release. **`php scripts/st_cc_normalized_preview_slice8_selftest.php`** asserts admin run-detail preview never exposes the full **`packages`** array (only **`packages_sample`**).

**Storage / retention (operator expectation):** each completed run appends **`credential_check_results`** and **≤32 KiB** stdout **`credential_check_artifacts`** per plugin row; **`normalized_json`** is capped (~2000 package dicts × field caps). **`package_inventory_observed`** upserts **one row per run** (`source_object_ref` includes `run_id`), so observation volume scales with **run history**, not package cardinality — prune old runs or trim **`asset_observations`** if retention becomes an issue (no automated TTL in MVP). **`software_observed`** is capped **per asset** for this plugin path (≤128 rows); full lists remain in results/artifacts only.
