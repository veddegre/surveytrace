...
- **`php scripts/convert_ubuntu_advisories.php`** — optional **`--fetch`** (Canonical OVAL bz2) or **local** OVAL / intermediate JSON → normalized import JSON; **`--import`** chains **`import_distro_advisories.php`** (explicit opt-in).
...
- `api/lib_ubuntu_advisory_convert.php`, `scripts/convert_ubuntu_advisories.php`, `scripts/st_convert_ubuntu_advisories_selftest.php` — Ubuntu advisory JSON normalization / OVAL conversion (offline by default; bounded)
