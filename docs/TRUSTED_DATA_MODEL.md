...
**`software_inventory*`** asset state is **long-lived**: inactive rows record “last seen” history; **do not** bulk-delete active/inactive state as part of routine observation pruning. **`software_observed`** (legacy) is removed on new inventories. Historical bounded snapshots remain in **`credential_check_results`** / artifacts — prune per operational retention when disk grows.
