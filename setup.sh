...
REQUIRED_PKGS=(
    ...
    python3-venv
    python3-dev
    # PHP + SQLite (php-xml = XMLReader; php-bz2 = compress.bzip2:// for Canonical *.xml.bz2 OVAL)
    php
    php-cli
    php-sqlite3
    php-json
...
# bz2 stream (compress.bzip2://) for Canonical *.xml.bz2 OVAL; meta php-bz2 may lag behind PHP_VER.
if apt-cache show "php${PHP_VER}-bz2" &>/dev/null; then
    apt-get install -y --no-install-recommends "php${PHP_VER}-bz2" || \
        warn "Could not install php${PHP_VER}-bz2 — OVAL --fetch may fail until it is installed"
fi
...
check_file "$INSTALL_DIR/api/lib_credential_schedule.php" "api/lib_credential_schedule.php exists"
check_file "$INSTALL_DIR/api/credential_profiles.php" "api/credential_profiles.php exists"
check_file "$INSTALL_DIR/api/cred_secret_helper_debug.php" "api/cred_secret_helper_debug.php exists"
...
check_file "$INSTALL_DIR/scripts/run_operational_integrity_suite.php" "scripts/run_operational_integrity_suite.php exists"
check_file "$INSTALL_DIR/scripts/check_database_integrity.php" "scripts/check_database_integrity.php exists"
...
check_readable_as_user "$WEB_GROUP" "$INSTALL_DIR/api/lib_credential_schedule.php" "www-data readable: lib_credential_schedule.php"
check_readable_as_user "$WEB_GROUP" "$INSTALL_DIR/api/credential_profiles.php" "www-data readable: credential_profiles.php"
check_readable_as_user "$WEB_GROUP" "$INSTALL_DIR/api/cred_secret_helper_debug.php" "www-data readable: cred_secret_helper_debug.php"
