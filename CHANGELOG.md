...
- **Vulnerability Dashboard** — `api/vulnerability_dashboard.php` read-only API (9 bounded actions: `summary`, `top_assets`, `recent_findings`, `aging`, `by_severity`, `by_package`, `by_advisory`, `suppressed`, `overrides`); per-asset weighted risk rollup (critical=40, high=10, medium=3, low=1; bands: critical/high/medium/low/none); dedicated UI tab with summary cards, highest-risk assets, aging criticals, common packages, recent findings, suppressions, and analyst overrides; asset detail host panel risk rollup banner; `vulnerability_dashboard` health block in `/api/health.php`; `scripts/diagnose_vulnerability_dashboard.php` CLI diagnostics; `scripts/st_vulnerability_dashboard_selftest.php` validates rollup math, counts, aging, bands, and bounded queries.
- **Operational integrity framework** — `scripts/run_operational_integrity_suite.php` unified read-only validation runner (lint, selftests, deploy coverage, DB integrity, runtime invariants, health, shell syntax); `scripts/check_database_integrity.php` standalone DB consistency checker (PRAGMA integrity_check, orphan detection, duplicate advisories, stale leases, malformed JSON, invalid status values, broken FK relationships); `scripts/st_operational_integrity_selftest.php` in-memory lifecycle regression (import→correlate→triage→suppress→dashboard→cleanup→assert); `scripts/diagnose_operational_integrity.php` JSON diagnostics (scheduler, worker, vulnerability, DB health, stale components); `operational_integrity` health block in `/api/health.php`; release gate integration in `release_security_gate.php`.
- **Automated correlation processing** — `surveytrace-vuln-correlation.timer` (every 5 min) + `surveytrace-vuln-correlation.service` (oneshot, bounded 240s deadline, 25-job batch, stale lease recovery); deploys as systemd timer enabled by `deploy.sh`/`setup.sh`; no overlapping runs (systemd prevents concurrency); logs via journal.
- **Advisory merge selftest coverage** — `st_vulnerability_correlation_selftest.php` extended with NVD→vendor upgrade, vendor→NVD enrichment without downgrade, NVD-only non-correlation, and `st_vuln_package_authority_merge` / `st_vuln_incoming_package_authority` helper assertions.
- **Vulnerability remediation workflow (Phase 1)** — `vulnerability_remediation_actions` table; `api/vulnerability_remediation.php` (CRUD + verify + close + dashboard_counts; CSRF, bounded, audit-logged); dashboard "Remediation status" section (overdue, unresolved >30d, verification failures, recently resolved); `vulnerability_remediation` health block; `scripts/diagnose_vulnerability_remediation.php` (JSON diagnostics); `scripts/prune_vulnerability_remediation_history.php` (dry-run retention); `scripts/st_vulnerability_remediation_selftest.php` (lifecycle, verification, overdue, prune, orphan, cascade tests).
...
Software Inventory Reconciliation **Foundations (slices 1–4)** on the trusted-data path: bounded **`software_observed`** observations from SSH package inventory, lazy **`software_inventory_summary`** assertions, Host modal evidence UX, **`trusted_data`** readiness diagnostics, and resolver explainability — **without** CVE matching, findings, or remediation.

### Added

- **Slice 1 — `software_observed`** — Bounded per-asset observation rows (cap **128**, replace semantics per **`ssh.linux.package_inventory`**); summarized **`package_inventory_observed`** retained.
...
- Resolver explanations emphasize inventory-only posture, future fusion posture (**scanner / API / SBOM / agent**) **without** new ingestion paths.

### Fixed

- None specific to this milestone (foundations shipped incrementally on main).
...
- **CVE matching**, **findings**, **remediation**, advisory fusion, per-package vulnerability assertions, SBOM/CPE authority — still **out of scope** for this release line ([TRUSTED_DATA_MODEL.md](docs/TRUSTED_DATA_MODEL.md), [CREDENTIALED_CHECKS_ENGINE.md](docs/CREDENTIALED_CHECKS_ENGINE.md)).
