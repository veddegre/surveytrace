...
**Source connectors** (e.g. Zabbix under **Enrichment** / **`/api/zabbix.php`**) use a different pattern than **`integrations`** rows: SQLite-backed cache, workers, matching, and preview/apply workflows. See **[Connector development guide](docs/CONNECTOR_DEVELOPMENT_GUIDE.md)** and the **[Roadmap](#roadmap)**.
...
- **Documentation** — README [Roadmap](#roadmap) reworked **upcoming** work into **named roadmap tracks** (not a fixed delivery order); added **[Connector development guide](docs/CONNECTOR_DEVELOPMENT_GUIDE.md)** (`docs/CONNECTOR_DEVELOPMENT_GUIDE.md`). **TeamDynamix** and **Microsoft Defender** called out as **deferred**, not next. Added **[UI cleanup plan](docs/UI_CLEANUP_PLAN.md)** (structured passes; no UI code changes). No implementation change.

### 0.16.0

- **Roadmap note** — **`VERSION` 0.16.0** is a **Phase 15 (Integrations)** follow-up (UI + legacy config cleanup), not the same as shipped **Zabbix monitoring enrichment** work summarized under [Completed](#completed-summary) (Phase 16.x). **`migration_phase16_remove_legacy_integrations_pull_token_v1`** is a SQLite migration **name** for ordering after the Phase 14.1 integration migrations; it does **not** mean all Zabbix roadmap items are delivered.
...
For how **source connectors** (scan-time plugins vs API-backed cache like Zabbix) should be structured end-to-end, see **[Connector development guide](docs/CONNECTOR_DEVELOPMENT_GUIDE.md)** and the **[Roadmap](#roadmap)** **Monitoring enrichment** / **Connector framework** tracks.
...
**Historical roadmap phase numbers** (Phase 1, Phase 2, … in [Completed](#completed-summary) below, including Phase 16.x for Zabbix) are a **planning index** for what has shipped. They are **not** the same as **`VERSION`** / SemVer: e.g. **`VERSION` 0.16.0** can ship while follow-on work in a theme (monitoring, UI, connectors) is still in progress.

**Upcoming work** is grouped into **named roadmap tracks** below. Tracks are **planning buckets** for related themes — **not a fixed delivery order**, not commitments per release, and items may move between tracks as priorities change. For a reusable connector pattern (config, workers, cache, matching, preview/apply, audit), see **[Connector development guide](docs/CONNECTOR_DEVELOPMENT_GUIDE.md)**.

**`migration_phaseN_*`** keys in **`api/db.php`** use integers for **ordering and grouping**; they often align with historical roadmap phases (e.g. **`migration_phase14_scan_scopes_v1`**) but not always — **`migration_phase16_remove_legacy_integrations_pull_token_v1`** is still **Integrations (Phase 15 scope)** cleanup, not “all Zabbix monitoring work.”
...
- **Data fusion and source reconciliation** — Multi-source CVE/advisory **dedupe**, **source weighting**, **conflict resolution**, **OSV / NVD / KEV / EPSS** and package/advisory expansion; reconciling **scanner** evidence with **other** vulnerability/telemetry sources when those connectors exist. *Depends on having multiple substantive sources in production.*
- **Credentialed checks engine** — Authenticated **in-host** collection (**SSH** / **WinRM** / **SNMPv3** / APIs where appropriate), plugin/check framework, richer package/version evidence, remediation metadata. **Distinct from** read-only **API-source** enrichment in the **ownership/endpoint** track: this track is about **executing checks** on endpoints, not only pulling vendor APIs.
- **Risk operations and governance** — Composite risk scoring, time-bound suppressions/exceptions, SLA tracking, stronger audit/report controls.

Shipped and in-progress **Zabbix** slices remain described historically as **Phase 16.1–16.4** under [Completed](#completed-summary).

### Completed (summary)

Per-release narrative is also in **`RELEASE_NOTES.md`** and [Changelog](#changelog) above.
...
- **Phase 16.3 — Scheduled Zabbix pull + freshness** — Optional **scheduled** JSON-RPC pull (**`sync_schedule_enabled`**, **`sync_interval_minutes`** default **60**, min **5**, max **1440**, **`next_sync_at`**) on **`zabbix_connector`**; **`daemon/scheduler_daemon.py`** spawns **`api/zabbix_sync_worker.php`** when the connector is **enabled**, **fully configured**, schedule **on**, and **`next_sync_at`** is due, using **`scheduled_sync_lock`** (separate from output push’s **`scheduled_output_lock`**). **Manual Sync now** remains; **`last_sync_started_at`** / **`last_sync_completed_at`** track runs. **Freshness** (**`fresh`**, **`stale`**, **`outdated`**, **`never_synced`**, **`disabled`**, **`not_configured`**) is derived from cache age vs the configured interval; exposed on **`GET /api/zabbix.php?status=1`**, **`GET /api/zabbix.php`** (**`schedule`**, **`freshness`**), and **`zabbix_enrichment_status`** on assets. **No** automatic scope, identity, or ownership changes — pull only refreshes cached Zabbix tables + rematch.
...
**HTTP behavior** — Each JSON-RPC call uses a **short cURL timeout** (~12s connect capped). Sync pulls at most **500** hosts and **4000** problem rows per run; counts may be incomplete if the server exceeds those caps. **`host.get`** requests both **`selectGroups`** (older Zabbix → `groups`) and **`selectHostGroups`** (Zabbix 7+ → `hostgroups`) so membership is written to **`zabbix_host_groups`** and **`groups_json`** reliably.

**GET diagnostics** — **`GET /api/zabbix.php`** includes **`zabbix_cache_status`** (host row count, **`zabbix_host_groups`** row count, hosts with non-empty **`groups_json`**, and **`scope_map_catalog`** group/tag counts plus a **`status_hint`** string) to debug empty group dropdowns without exposing secrets.
...
**Stored vs displayed** — **Stored** in SQLite: full connector row (token on disk), normalized **`zabbix_*`** tables plus JSON blobs on **`zabbix_hosts`** (`inventory_json`, `groups_json`, `tags_json`, `templates_json`, `interfaces_json`, `status_raw_json`) for audit/re-sync. The **`zabbix_hosts`** row uses **`tech_name`** (Zabbix JSON **`host`** — technical name) and **`visible_name`** (Zabbix JSON **`name`** — visible/display name); sync maps API `host` → `tech_name` and `name` → `visible_name` (if `name` is empty, `visible_name` is set to the technical name). **`status_raw_json`** also retains `host` and `name` for repair on resync if columns were ever blank. **`zabbix_asset_links`** stores **`match_method`**, **`confidence`**, **`last_matched_at`**. **`GET /api/zabbix.php?match_review=1`** joins **`zabbix_hosts`** and returns **`zabbix_tech_name`**, **`zabbix_visible_name`**, and **`zabbix_display_name`** (visible, else technical, else `hostid`) so linked rows are not mistaken for SurveyTrace **`assets.hostname`**. The asset API **`zabbix`** object includes **`tech_name`**, **`visible_name`**, and **`display_name`** when linked. List sizes in the API response are **capped** (e.g. groups/tags/templates) to keep payloads bounded; the UI truncates long joined strings in the asset modal.

**Read-only vs explicit writes** — Zabbix enrichment on **`GET /api/assets.php?id=`** and the host **Details** panel is **read-only** for trust fields and names until an operator runs an explicit workflow. **Scope mapping** only ever changes **`assets.scope_id`** (preview → confirm → apply, audited **`zabbix.scope_map_applied`**). **Identity apply** only changes **`assets.hostname`** when it is still **blank** and **not locked** ( **`POST preview_identity_apply`** / **`POST apply_identity`**, audited **`zabbix.identity_applied`**). On apply, SurveyTrace also sets **`hostname_locked = 1`**, **`identity_confidence = 1.0`**, and **`identity_confidence_reason = 'zabbix_identity_applied'`** so later scans do not overwrite the Zabbix-sourced name. SurveyTrace does **not** auto-fill hostname from Zabbix on sync.
...
**Scope mapping workflow** — 1) **Sync Zabbix** so **`scope_map_catalog`** (groups/tags) and host links exist. 2) Save rules (each row **disabled** until you enable it), choosing patterns from the catalog or custom text. 3) **Preview mapping** (form rules) or **Build apply plan** (saved **enabled** rules only) to see **current scope → new scope**. 4) Check the explicit confirmation box and **Apply selected rows** — the server rejects stale or mismatched rows; **`zabbix.scope_map_applied`** is written to the audit log when anything is applied. The apply-plan table supports **Select all / Clear all** (and a header control) for eligible rows only; **`POST apply_scope_map`** returns **`skip_reasons`** counts (e.g. **`already_target_scope`**) so the UI can summarize outcomes — **the server still revalidates every selected row** before writing.

**Identity apply workflow (hostname)** — 1) **Sync** and confirm the asset is **linked** to the intended Zabbix host. 2) Under Enrichment → Zabbix tools, **Build identity plan** (`preview_identity_apply`) to list assets with a **blank** hostname, **unlocked** hostname field, and a usable Zabbix visible/technical name. 3) Select rows (same **Select all / Clear all** pattern as scope apply) and confirm **Apply selected identity updates** (`apply_identity` with `confirm: true`). The server re-checks the preview snapshot and Zabbix names; **`skip_reasons`** summarizes skips (**`already_set`**, **`hostname_locked`**, **`no_zabbix_name`**, **`not_linked`**, **`stale_current_hostname`**, **`stale_preview`**, **`not_selected`**, **`invalid_row`**). There is **no** “overwrite non-empty hostname” option in this release.

**Zabbix output (SurveyTrace → Zabbix)** — Optional output push sends compact numeric health/security counters (for example scan status, active jobs, ingest backlog, open critical/high findings, collector health, and asset totals) to a configured Zabbix host with configurable key prefix (default **`surveytrace.`**). Output must be explicitly enabled and configured in **Integrations → Zabbix**. Scheduler pushes are lightweight and periodic; manual **Send test metrics** is available for validation. SurveyTrace does **not** create Zabbix triggers automatically in this phase.

**Hostname precedence (scanner + inventory)** — When several names exist (PTR aliases, mDNS, NetBIOS, nmap hostnames), SurveyTrace **ranks** candidates instead of blindly using one label (which used to collapse names like **`pi.hole`** to **`pi`** and could regress a better hostname). **Precedence:** manual / operator-approved (**`hostname_locked`**, or **`identity_confidence` ≥ 0.75** from operator edits / enrichment) **>** Zabbix identity apply (see above) **>** stable stored hostname **>** new discovery **>** generic service-style names (**`pi.hole`**, **`localhost`**, **`gateway`**, **`router`**, **`unifi`**, **`wlan0`**, **`device-*`**, **`amazon-*`**, etc.). The scanner **does not overwrite** a locked hostname and **does not replace** trusted non-empty hostnames unless the stored value is clearly generic and the new candidate ranks substantially higher.

**Manual hostname repair (SQLite)** — If an asset regressed before this behavior was enabled, repair explicitly (adjust IP/hostname as needed); **do not** auto-run in pipelines:
...
SET hostname='fjord',
    ...
    identity_confidence_reason='manual_hostname_repair'
WHERE ip='192.168.23.10';
```

**Asset list filters** — When workflow migrations are applied, the Assets tab can filter by Zabbix monitoring, unavailable (linked but not “available”), open problems, and by **host group** or **tag** (same semantics as rules). Optional **Zbx column** shows a compact trust summary. **Inventory scope** — Assets can be **filtered** (`GET /api/assets.php?scope_id=…`, **`export.php`** same param) and **sorted** (`sort=scope_name`) by **`assets.scope_id`** / scope name; this is **live inventory** only. **Reports & Analysis** still filters completed scan jobs by **`scan_jobs.scope_id`** (job scope at queue time); that is **independent** from asset inventory scope and does not rewrite historical snapshots when you change asset tags. **Needs AI review** — **`ai_review=1`** lists actionable scan-time classification signals only: **`ai_last_suggested_category`** differs from the stored **`category`**, or **`ai_last_reason`** is set after a scan-AI attempt to something other than benign skips (`same_category`, `suggest_only`, `low_confidence`, `strong_port_signal`), or **`identity_confidence` &lt; 0.75**. It does **not** treat operator **host summary** text (`ai_host_explain_cache`) as review work. **Has host summary** — optional **`ai_summary=1`** (under **AI filters** in the UI when the cache column exists) matches saved operator summaries with **`status=ok`** in **`assets.ai_host_explain_cache`**.
...
This section is **design intent only** for the **Ownership and endpoint enrichment** roadmap track — **TeamDynamix** and **Microsoft Defender** are **not implemented**, are **deferred** (see [Upcoming roadmap tracks](#upcoming-roadmap-tracks)), and are **not** the next required releases. Nothing here ships until called out in [Changelog](#changelog) / **`RELEASE_NOTES.md`**. **Zabbix** (Phase 16.1–16.4 under [Completed](#completed-summary)) is the **live reference** for connector principles: **bounded sync**, **background workers**, **redacted secrets**, **explicit operator actions** for anything that changes ownership or scope, and **normalized rows** in SQLite — see the **[Connector development guide](docs/CONNECTOR_DEVELOPMENT_GUIDE.md)** (reference table at the end maps topics to Zabbix artifacts). **Splunk** (or similar SIEM) remains the **correlation / primary alerting** layer — SurveyTrace stores normalized enrichment and can **push events**, not replace SIEM time-window joins.
...
## Zabbix output (SurveyTrace → Zabbix)

**Pull vs output** — **Zabbix pull** (JSON-RPC sync / scheduled pull) copies Zabbix host data **into** SurveyTrace’s local cache for enrichment UI and matching. **Zabbix output** (this section) sends SurveyTrace summary metrics **to** Zabbix via **`zabbix_sender`**; it does not replace or drive the pull schedule.
...
| surveytrace.scans.active_jobs | Number of active scan jobs |
| surveytrace.ingest.backlog | Collector ingest backlog size |
| surveytrace.vuln.critical_open | Open critical vulnerabilities |
| surveytrace.vuln.high_open | Open high vulnerabilities |
| surveytrace.collectors.active | Active collectors |
| surveytrace.collectors.offline | Offline collectors |
| surveytrace.assets.total | Total assets |
| surveytrace.assets.stale | Stale assets |
