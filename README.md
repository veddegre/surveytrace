...
- **System health** — **System** sidebar tab and `GET /api/health.php` (data dir, free space via `df` when available, DB file sizes via `stat`, optional systemd for `surveytrace-daemon` / `surveytrace-scheduler`, scan queue, last feed job)
