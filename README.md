...
- **System health** — **System** sidebar tab and `GET /api/health.php` (data dir, free space from `df -kP` the same 1K-block *Available* as plain `df`, DB sizes via `stat`/`filesize`, optional systemd for `surveytrace-daemon` / `surveytrace-scheduler`, scan queue, last feed job)
